...
<?php
	...
	if (!is_null($_GET["player"])){ // update player if existing
		...
		unlink("player/$rsn.json");
		file_put_contents("player/$rsn.json", $JSONE);
		echo "$log";
	}

	if (!is_null($_POST["player"])){ // update player if existing
		$rsn = $_POST["player"];
		$JSONE = getPlayerStats($rsn);
		unlink("player/$rsn.json");
		file_put_contents("player/$rsn.json", $JSONE);
	...
	if (!is_null($_GET["ml"])){ // update memberlist if existing
		$ml = $_GET["ml"];
		$fileText = fopen(dirname(getcwd())."/ml/$ml", "r");
		if ($fileText){
		...
		fclose($fileText);
		unlink("$ml.json");
		file_put_contents("$ml.json", $JSONE);
		echo "$JSONE";
